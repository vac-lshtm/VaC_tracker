...
s$n_country[s$Institutes=="Biological E Corbevax"] = sum(grepl("Corbevax", owid$vaccines))
...
s$n_country[s$Institutes=="Chumakov Center CoviVac"] = sum(grepl("KoviVac/Chumakov", owid$vaccines))
...
g3 = ggplot(s, aes(as.numeric(n_country), Institutes, fill = Platform, label = n_country)) + geom_bar(stat = "identity") + theme_bw() +
  ...
  scale_x_continuous(limits=c(0,240), breaks=c(0,100,200)) +
