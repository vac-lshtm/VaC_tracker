...
if (!("mx_reduced.csv" %in% list.files())) {
  ...
  mx_results <- mx_search(data = mx_snapshot(), query = query, from_date = "2020-01-01", to_date = "2021-04-19") ### UPDATE ###
...
if (!("pubmed_5.csv" %in% list.files())) {
  query <- "(coronavirus OR COVID OR SARS*) AND vaccin* AND (trial OR phase) AND 2021/02/02:2021/04/19[crdt]" ### UPDATE ###
  pm_download <- batch_pubmed_download(pubmed_query_string = query, encoding = "ASCII")
...
write.csv(collated, "pre_abstract_screening_19Apr2021_full.csv")  ### UPDATE ###
#system("rm pubmed_*")
