...
if (!("mx_reduced.csv" %in% list.files())) {
  ...
  mx_results <- mx_search(data = mx_snapshot(), query = query, from_date = "2020-01-01", to_date = "2021-08-02") ### UPDATE ###
...
if (!("pm_df.csv" %in% list.files())) {
  query <- "(coronavirus OR COVID OR SARS*) AND vaccin* AND (trial OR phase) AND 2021/06/16:2021/08/02[crdt]"  ### UPDATE
...
write.csv(collated, "pre_abstract_screening_02Aug2021_full.csv")  ### UPDATE ###
#system("rm pubmed_*")
